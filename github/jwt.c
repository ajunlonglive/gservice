...
static int64_t get_js_int(json_t *js, const char *key) {
	int64_t val = -1;
	json_t *js_val;
...
void *jwt_b64_decode(const char *src, int *ret_len) {
	...
	bin_len = pg_b64_dec_len(i);
	buf = (char *)malloc(bin_len + 1);
	if (buf == NULL)
		return NULL;

	rc = pg_b64_decode(new, i, buf);
	if (-1 == rc) {
		free(buf);
		return NULL;
	}
...
int64_t jwt_get_grant_int(jwt_t *jwt, const char *grant) {
	if (!jwt || !grant || !strlen(grant)) {
		errno = EINVAL;
		return 0;
	}
...
int jwt_add_grant_int(jwt_t *jwt, const char *grant, int64_t val) {
	if (!jwt || !grant || !strlen(grant))
		return EINVAL;
...
int64_t jwt_get_header_int(jwt_t *jwt, const char *header) {
	if (!jwt || !header || !strlen(header)) {
		errno = EINVAL;
		return 0;
	}
...
int jwt_add_header_int(jwt_t *jwt, const char *header, int64_t val) {
	if (!jwt || !header || !strlen(header))
		return EINVAL;
	...
	if (json_object_set_new(jwt->headers, header, json_integer(val)))
		return EINVAL;
...
static int jwt_encode(jwt_t *jwt, char **out) {
	...
	bin_len = strlen(buf);
	b64_len = pg_b64_enc_len(bin_len);
	head = alloca(b64_len + 1);
	if (head == NULL) {
		SE_free(buf);
		return ENOMEM;
	}
	head_len = pg_b64_encode(buf, bin_len, head);
	if (-1 == head_len) {
		free(buf);
		return head_len;
	}
	...
	bin_len = strlen(buf);
	b64_len = pg_b64_enc_len(bin_len);
	body = alloca(b64_len + 1);
	if (body == NULL) {
		SE_free(buf);
		return ENOMEM;
	}
	body_len = pg_b64_encode(buf, bin_len, body);
	if (-1 == body_len) {
		SE_free(buf);
		return body_len;
	}
	...
	if (ret)
		return ret;
	b64_len = pg_b64_enc_len(sig_len);
	buf = malloc(b64_len +1);
	if (buf == NULL) {
		free(sig);
		return ENOMEM;
	}
	body_len = pg_b64_encode(sig, sig_len, buf);
	if (-1 == body_len) {
		free(sig);
		free(buf);
		return body_len;
